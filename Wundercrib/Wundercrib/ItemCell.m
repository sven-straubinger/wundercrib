...
- (void)setResolved:(BOOL)resolved
{
    ...
    // Disallow textfield user interaction on a resolved item
    [self.textfield setUserInteractionEnabled:!resolved];
...
{
    // Determines, if pan gestures are allowed simultaneously, e.g. with scroll view gesture recognizer
    return self.gestureRecognizersAllowedSimultaneously;
}

- (void)panGesture:(UIPanGestureRecognizer*)panGesture
{
    // Forward result to delegate (view controller)
    if([self.delegate respondsToSelector:@selector(itemCell:detectedPanGestureWithRecognizer:)]){
        [self.delegate itemCell:self detectedPanGestureWithRecognizer:panGesture];
    }
}
